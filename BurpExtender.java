...
import burp.JSONException;
import burp.JSONObject;
...
public class BurpExtender extends AbstractTableModel implements ISessionHandlingAction ,ITab, IMessageEditorController
{
    ...
    public void registerExtenderCallbacks(final IBurpExtenderCallbacks callbacks)
    {
        ...
        //new PrintWriter(callbacks.getStdout(), true);
    ...
    try {
        ...
        boolean bearer_token = json.has("access_token");
        String Bearer = null; 
        ...
        for (int i = 0; i < headers.size(); i++){
           if(((String) headers.get(i)).startsWith("Authorization: Token "))
                 headers.remove(i);
        }          
        
        headers.add("Authorization: Token " + Bearer);   
        ...
        {
            int row =log.size();
            log.add(new LogEntry("adding new header  - Authorization Token: " + Bearer,"Header Checked at time : " + date, callbacks.saveBuffersToTempFiles(currentRequest)));
